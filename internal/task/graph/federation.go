...
func (ec *executionContext) resolveEntity(
	...
	case "Task":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Task": %w`, err)
		}
		switch resolverName {
		...
		case "findTaskByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findTaskByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindTaskByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Task": %w`, err)
			}
...
func entityResolverNameForTask(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Task", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Task", ErrTypeNotFound))
			break
		}
		return "findTaskByID", nil
	}
	return "", fmt.Errorf("%w for Task due to %v", ErrTypeNotFound,
